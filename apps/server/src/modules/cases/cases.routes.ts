...
import { emitCaseCreated, emitCaseUpdated } from "../../realtime/socket.js";
...
casesRouter.post("/", requireAdmin, validateBody(createCaseSchema), async (req, res, next) => {
  try {
    const kase = await casesService.createCase(body<CreateCaseInput>(req), req.user!.id);
...
casesRouter.patch("/:id", requireAdmin, validateBody(updateCaseSchema), async (req, res, next) => {
  try {
    const id = cuidSchema.parse(req.params.id);
    const kase = await casesService.updateCase(id, body<UpdateCaseInput>(req), req.user!.id);
    emitCaseUpdated(id);
    res.json(kase);
  } catch (err) {
    next(err);
